        ...
        <div class="border-b border-gray-900/10 pb-12">
            ...
            <x-form-box for="first_name"> First Name*
                <x-form-input type="text" name="first_name" id="first_name"
                    value="{{ old('first_name') ?? (request()->input('first_name') ?? $contact?->first_name) }}"></x-form-input>
            ...
            <x-form-box for="last_name"> Last Name*
                <x-form-input type="text" name="last_name" id="last_name"
                    value="{{ old('last_name') ?? (request()->input('last_name') ?? $contact?->last_name) }}"></x-form-input>
            ...
            <x-form-box for="email"> Email Address*
                <x-form-input type="email" name="email" id="email"
                    value="{{ old('email') ?? (request()->input('email') ?? $contact?->email) }}"></x-form-input>
            ...
            <x-form-box for="phone_number"> Phone Number
                <x-form-input type="text" name="phone_number"
                    id="phone_number"
                    value="{{ old('phone_number') ?? (request()->input('phone_number') ?? $contact?->phone_number) }}"></x-form-input>
            ...
            <x-form-box for="notes"> Notes
                <x-hint>Internal use notes about this contact (will not export
                    to CSV)</x-hint>
                <x-form-input type="text" name="notes" id="notes"
                    value="{{ old('notes') ?? (request()->input('notes') ?? $contact?->notes) }}"></x-form-input>
